...
if __name__ == '__main__':
    ...
    raw_dat.shape
    raw_dat.count() # checks how many missing values are there
    raw_dat.head()
    raw_dat.columns
    ...
    # Drops the non important variables according to a cut value
    feats = dat[important_vars[1:50]] # TODO: automate choice of optimal cut
    ...
    # Training
    clf = DecisionTreeClassifier(max_depth = 50) # TODO: should grow the
    # tree and then prune it automatically according to an optimal depth
    ...
    #### Model 2 | Random Forest Model (Extremely Random Forests) ####

    print "Model 2 executing..."

    # Training

    # Balances the dataset, otherwise RF doesn't work well for classification
    pos = train[y_train == 1]
    ...
    clf = ExtraTreesClassifier(n_estimators = 500, verbose = 1,
        bootstrap = True, max_depth = None, oob_score = True, n_jobs = -1)
    ...
    aux = np.array([0] * len(y_all_models))
    aux[y_all_models > 1] = 1
    perf_model_ensemble = Performance.get_perf(y_test, aux)

    # Confusion Matrix
    print pd.crosstab(
        y_test, perf_model_ensemble, rownames = ['actual'], colnames = ['preds'])
    ...
    print pd.DataFrame([[col1, col2, col3 * 100] for col1, d in
